...
function error(int $errno, string $errmsg, string $errfile, int $errline) {
	...
	// AJAX error
	} elseif ($GLOBALS['_panel'] = 'ajax') { 
		$response = array('status' => 'error', 'errmsg' => $errmsg, 'file' => $errfile, 'line' => $errline);
		echo json_encode($response); exit(0);

// Check if 500.tpl error template exists
	} elseif (!file_exists(SITE_PATH . '/data/tpl/' . $GLOBALS['_panel'] . '/500.tpl')) {
...
function get_table_details($table, string $table_id = '') {

	// Set variables
	$page = $_REQUEST['page'] ?? 1;
	$search_text = $_POST['search_' . $table_id] ?? '';
	$order_by = isset($_POST['sort_col']) && isset($_POST['sort_dir']) ? $_POST['sort_col'] . ' ' . $_POST['sort_dir'] : '';

	// Get total rows
	$total = method_exists($table, 'get_total') ? $table->get_total($search_text) : 0;
	...
	// Get rows
	$rows = $order_by == '' ? $table->get_rows($start, $search_text) : $table->get_rows($start, $search_text, $order_by);
